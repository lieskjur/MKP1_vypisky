...
\begin{align*}
	U^e &= \frac{1}{2} \int\limits_0^l E \epsilon^2(\xi) A d\xi
		%= \frac{1}{2} \int\limits_0^l \epsilon(\xi) E A\,\epsilon(\xi) d\xi
...
%Pole posuvů $\ul{u}$ a deformací $\ul{\epsilon}$ elementu lze vyjádřit jako funkci souřadnic $x,y$ a vektoru uzlových posuvů elementu $\ul{\delta}^e$:
%
\begin{align*}
...
%\begin{equation*}
%	U	= \frac{1}{2} \int\limits_\Omega \ul{\epsilon}^T \ull{E}\ \ul{\epsilon} \ dV
...
%\begin{equation*}
%	\tenz{\epsilon}_0 = \alpha \Delta T \begin{bmatrix}1&0&0\\0&1&0\\0&0&1\end{bmatrix}
%\end{equation*}
%
\begin{align*}
...
U = \int\limits_\Omega \Lambda \ dV
	...
	%= \frac{1}{2}\,\ul{\delta}^e \ull{K}^e \ul{\delta}^e - \frac{1}{2}\,\ul{\delta}^e\!\int\limits_\Omega \ull{B}^{e^T}\!\ull{E}\ \ul{\epsilon}_0 \ dV
\end{equation*}
%
